...
ggplot(data = dat,aes(as.ordered(physicalHealthRev),as.integer(depressionRev)))+geom_boxplot()
```
...
ggplot(dat, aes(depressionRev,color=marital,fill=marital)) + geom_density(alpha=.5)
```

Individuals who are married will experience lower levels of of anxiety
```{r}
kruskal.test(anxietyRev ~ marital, data= dat)
ggplot(dat, aes(anxietyRev,color=marital,fill=marital)) + geom_density(alpha=.5) 
```
